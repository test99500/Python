...
ds_train = ds_train.batch(1)# 128 works on GPU too but comes very close to the memory limit of the Colab GPU. [1]
ds_train = ds_train.prefetch(AUTO)

# Build evaluation pipeline
ds_test = ds_test.map(preprocessing, num_parallel_calls=AUTO)
ds_test = ds_test.batch(1) # On Colab/GPU, a higher batch size does not help and sometimes does not fit on the GPU (OOM).[2]
ds_test = ds_test.prefetch(AUTO)

# Plug the input pipeline into Keras.
model = Sequential([
    Conv2D(filters=96, kernel_size=(11, 11), strides=(4, 4), activation=tf.nn.relu,
           data_format='channels_last', input_shape=(227, 227, 1)),
    BatchNormalization(),
