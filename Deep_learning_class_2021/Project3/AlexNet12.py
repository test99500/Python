...
ds_train = ds_train.batch(4)# 128 works on GPU too but comes very close to the memory limit of the Colab GPU. [1]
ds_train = ds_train.prefetch(AUTO)

# Build evaluation pipeline
ds_test = ds_test.map(preprocessing, num_parallel_calls=AUTO)
ds_test = ds_test.batch(4) # On Colab/GPU, a higher batch size does not help and sometimes does not fit on the GPU (OOM).[2]
