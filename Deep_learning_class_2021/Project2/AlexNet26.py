...
model.compile(loss='sparse_categorical_crossentropy', optimizer=SGD(learning_rate=0.001),
              metrics=['accuracy'])
...
model.fit(train_ds,
          epochs=50) # validation_split=0.3 is only supported in numpy array.
...
y_prediction2 = model.predict(x=test_ds)

print(classification_report(y_true=test_labels, y_pred=y_prediction2, target_names=CLASS_NAMES))
